...
Так может получиться, если один из классов находится в чужой библиотеке. Но даже если в вашей. IName это явно утилитарный внутренний аспект поведения: хочется его скрыть, а не делиться друими:
...
| LengthLimit | Требует, чтобы строка или коллекция были заданной длины |  |
| SkipEmpty | Выкидывает из набора пустые значения |  |
| ToLower | Опускает регистр всех символов в строке | "aBcD" -> "abcd" |
| ToUpper | Поднимает регистр всех символов в строке | "aBcD" -> "ABCD" |
| Email | Проверяет, что строка - это корректный email | |
| Url | Проверяет, что строка - это URL | |
| FileName | Превращает строку в корректное имя файла | "aB$#cd" -> "aB__cd" |
| LatinFileName | Превращает строку в корректное имя файла, состоящее из латинских букв | "aФ$#cШ" -> "a___c_" |
| Enum | Проверяет, что объект определен в перечислении | |
| Forbidden | Запрещает передавать значения из переданного набора | |
