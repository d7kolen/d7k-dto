...
В то же время не публичные интерфейсы использовать не рекомендуется. Дело в том, что библиотека создает по мере необходимости на основе интерфейсов классы в Runtime режиме через наследование этих интерфейсов. От непубличного интерфейса нельзя будет отнаследоваться и при попытке создания класса произойдет Exception. **Возможно в будущем** эта проблема будет устранена и непубличные интерфейсы использовать будет возможно.
